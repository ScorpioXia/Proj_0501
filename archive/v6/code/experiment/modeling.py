...
def importance_summary(importances: pd.DataFrame, validation_splits: int) -> pd.DataFrame:
    ...
    working = importances.copy()
    if "repeat_seed" in working.columns:
        working["validation_split_id"] = (
            working["repeat_seed"].astype(str) + "__fold_" + working["outer_fold"].astype(str)
        )
        split_column = "validation_split_id"
    else:
        split_column = "outer_fold"
    return (
        working.groupby(["experiment_id", "model", "feature_set", "feature"], as_index=False)
        .agg(
            selected_folds=(split_column, "nunique"),
        ...
        .assign(selection_frequency=lambda frame: frame["selected_folds"] / validation_splits)
        .sort_values(
